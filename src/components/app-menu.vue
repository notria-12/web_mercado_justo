...
export default {
  ...
  data() {
    return {
      menuItems: authProtectedRoutes
        .filter(authRoute => authRoute.name !== 'profile'),
    }
  },
  computed: {
    ...authComputed,
  ...
  methods: {
    ...
    filterByPermission(menuItems) {
      if (this.user && this.user.tipo_conta === 'admin') {
        return menuItems
      } else {
        const userPermissions = this.user && this.user.permissoes
          ? this.user.permissoes
          : []
        let filteredMenu = []

        for (const menuItem of menuItems) {
          if (!menuItem.permission && !this.hasItems(menuItem)) {
            filteredMenu.push(menuItem)
          } else if (!menuItem.permission && this.hasItems(menuItem)) {
            let children = []
            for (const child of menuItem.children) {
              if (!child.permission) {
                children.push(child)
              } else if (child.permisssion && userPermissions.includes(child.permisssion)){
                children.push(child)
              }
            }
            filteredMenu.push({
...
<template>
  ...
  <ul id="side-menu" class="metismenu">
    <li
      v-for="item in filterByPermission(menuItems)"
      :key="`item-${item.name}`"
